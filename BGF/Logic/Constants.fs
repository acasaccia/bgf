...
let entitiesRemovalClampX = ( Shared.ContentData.viewPort.AspectRatio / 2.0f + Shared.ContentData.viewPort.AspectRatio / 100.0f * 5.0f ) * m;
let entitiesRemovalClamp = { X = entitiesRemovalClampX; Y = 0.55f<m> }
let viperClampX = ( Shared.ContentData.viewPort.AspectRatio / 2.0f - Shared.ContentData.viewPort.AspectRatio / 100.0f * 5.0f ) * m;
...
let cylonBoundingRadius = ( Shared.ContentData.raiderBoundingRadius * 0.8f ) * m // it's ugly, but must compensate for raider not really spherical shape
...
let vipershields = 5
let viperBoundingRadius = ( Shared.ContentData.viperMarkIIBoundingRadius * 0.8f ) * m
